...
const OverlayCard = ({ noteDetails }) => {
  return (
    <>
      <div
        className={`bg-[#1a1a1a] border border-solid border-borderColor p-4 w-[400px] opacity-40 h-[200px] flex flex-col justify-between rounded-xl`}
      >
        <p
          className={`text-2xl text-white mb-2 ${breakWordCheck(noteDetails?.title) ? "break-all" : ""}`}
        >
